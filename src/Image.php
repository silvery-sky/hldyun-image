...
class Image
{
    ...
    protected $error_type=[
        'illegal_image_file'=>'Illegal image file',
        'failed_to_create'=>'Failed to create image resources!',
        'file_not_exist'=>'image file not exist',
        'not_support_flip'=>'不支持的翻转类型',
        'not_support_thumb'=>'不支持的缩略图裁剪类型',
        'water_not_exists'=>'水印图像不存在',
        'error_water'=>'非法水印文件',
        'not_support_position'=>'不支持的水印位置类型',
        'not_exist_font'=>'不存在的字体文件',
        'not_support_font_position'=>'不支持的文字位置类型',
        'error_color'=>'错误的颜色值'
    ];
    ...
    protected function __construct(\SplFileInfo $file)
    {
        ...
        if (empty($this->im)) {
            $this->error='failed_to_create';
        }

    }

    public function getError(){
        if(isset($this->error_type[$this->error])){
            return $this->error_type[$this->error];
        }else{
            return '未知的错误';
        }
    }
    ...
    {
        if (!is_file($source)) {
            $this->error='water_not_exists';
        }
        //获取水印图像信息
        $info = getimagesize($source);
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            $this->error='error_water';
        }
        //创建水印图像资源
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    $this->error='not_support_position';
                }
        }
        do {
            //添加水印
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    $this->error='not_support_font_position';
                }
        }
        /* 设置偏移量 */
        if (is_array($offset)) {
